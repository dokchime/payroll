...
class SalaryStructureGrades extends DB
{
    ...
    public function createGrade($salary_structure_id, $grade_level, $step)
    {
    ...
    public function updateGrade($id, $struct_name, $grade_level, $step)
    {
        ...
        $stmt->bind_param("iiii", $struct_name, $grade_level, $step, $id);
    ...
    {
        $stmt = $this->conn->prepare("SELECT ssg.id AS ssg_id, ss.id AS ss_id, ssg.*,  ss.* FROM $this->table  ssg INNER JOIN salary_structure ss on ssg.salary_structure_id=ss.id order by ss.struct_name LIMIT ? OFFSET ?");
